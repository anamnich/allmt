...
local teleportPoints = {
    Vector3.new(1269, 641, 1790),
    Vector3.new(-6621, 3147, -795)
}
...
local autoRespawnEnabled = false -- 🔄 NEW FEATURE
		...
		while isTeleporting do
			for i = currentIndex, #teleportPoints do
				...
				if i == #teleportPoints then
					if autoResetCheckpoint then
						task.wait(0.5)
						currentIndex = 1
						teleportToPosition(teleportPoints[currentIndex])
						speedLabel.Text = "🔁 Restarted to Checkpoint 1"
					end
					-- 🔄 Auto respawn saat titik terakhir
					if autoRespawnEnabled then
						task.wait(0.5)
						respawnCharacter()
						speedLabel.Text = "🔄 Auto Respawn Triggered"
					end
				end
			end
			currentIndex = 1
...
local function createTeleporterUI()
	...
	-- 🔄 AUTO RESPAWN TOGGLE BUTTON
	local respawnToggle = Instance.new("TextButton", frame)
	respawnToggle.Size = UDim2.new(0,200,0,30)
	respawnToggle.Position = UDim2.new(0,20,0,205)
	respawnToggle.Text = "🔄 Auto Respawn: OFF"
	respawnToggle.BackgroundColor3 = Color3.fromRGB(255,120,120)
	...
	respawnToggle.MouseButton1Click:Connect(function()
		...
		respawnToggle.Text = autoRespawnEnabled and "🔄 Auto Respawn: ON" or "🔄 Auto Respawn: OFF"
		respawnToggle.BackgroundColor3 = autoRespawnEnabled and Color3.fromRGB(0,255,100) or Color3.fromRGB(255,120,120)
	end)

	local backBtn = Instance.new("TextButton", frame)
	backBtn.Size = UDim2.new(0,90,0,28)
	backBtn.Position = UDim2.new(0,20,0,240)
	backBtn.Text = "⏪ Back"
	...
	nextBtn.Position = UDim2.new(0,130,0,240)
	nextBtn.Text = "⏩ Next"
